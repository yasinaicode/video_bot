...
# نصب خودکار کتابخانه‌ها در سرور Render (فقط در اولین اجرا)
subprocess.check_call([sys.executable, "-m", "pip", "install", "python-telegram-bot", "yt-dlp", "instaloader"])
...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "youtube.com" in link or "youtu.be" in link:
        await update.message.reply_text("🎬 در حال دانلود از یوتیوب (بهترین کیفیت)...")
...
async def download_youtube(update: Update, link: str):
    ...
    try:
        ...
        await update.message.reply_document(document=open(filename, 'rb'), timeout=300)
        os.remove(filename)
